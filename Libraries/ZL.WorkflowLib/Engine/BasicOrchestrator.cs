...
namespace ZL.WorkflowLib.Engine
{
    ...
    public sealed class BasicOrchestrator : IOrchestrator
    {
        ...
        private OrchTaskResult ExecuteTask(OrchTask task, StepContext baseCtx, CancellationToken planToken)
        {
            ...
            for (int attempt = 1; attempt <= attempts; attempt++)
            {
                ...
                try
                {
                    ...
                    taskResult.CompletedAtUtc = DateTime.UtcNow;
                    return taskResult;
                }
        ...
        private Dictionary<string, object> ExecuteOnce(OrchTask task, StepContext baseCtx, CancellationToken planToken)
        {
            Func<Dictionary<string, object>> runCore = () =>
                DeviceLockRegistry.WithLock(task.Device ?? string.Empty, () => RunOnDevice(task, baseCtx, planToken));
        ...
        private Dictionary<string, object> RunOnDevice(OrchTask task, StepContext baseCtx, CancellationToken planToken)
        {
            ...
            using (var linked = CancellationTokenSource.CreateLinkedTokenSource(planToken))
            {
                ...
                var model = baseCtx != null ? baseCtx.Model : (DeviceServices.Config.Model ?? string.Empty);
                ...
                return DeviceServices.Factory.UseDevice(task.Device, deviceConfig, dev =>
                {
    ...
    /// <summary>
    /// <para>编排计划：由若干任务节点构成的有向无环图（DAG）。</para>
    /// </summary>
    public sealed class OrchestrationPlan
    {
        /// <summary>
        /// <para>计划名称，用于日志展示。</para>
        /// </summary>
    ...
    {
        /// <summary>
        /// <para>尝试次数，含首轮执行。</para>
        /// </summary>
        public int Attempts { get; set; }

        /// <summary>
        /// <para>两次尝试之间的间隔，单位毫秒。</para>
        /// </summary>
        public int DelayMs { get; set; }
    }

    /// <summary>
    /// <para>窗口配置（当前实现未使用，预留给未来扩展）。</para>
    /// </summary>
    ...
    public sealed class OrchTask
    {
        ...
        /// <summary>具体执行的设备名称。</summary>
        public string Device { get; set; }

        /// <summary>设备命令。</summary>
        public string Command { get; set; }

        /// <summary>命令参数集合。</summary>
        public Dictionary<string, object> Parameters { get; set; }

        /// <summary>资源互斥键，同一键的任务会被串行化。</summary>
        public string ResourceId { get; set; }

        /// <summary>依赖任务 Id 列表。</summary>
        public List<string> DependsOn { get; set; }

        /// <summary>单次执行的超时限制，毫秒。</summary>
