...
namespace ZL.WorkflowLib.Workflow
{
    ...
    internal sealed class SequentialOrchestrator : IOrchestrator
    {
        ...
        private static OrchTaskResult ExecuteTask(OrchTask task, StepContext sharedCtx)
        {
            ...
            var step = task.Step;
            UiEventBus.PublishLog($"---[SubFlow] 开始 {step.Name}, 设备【{step.Target}】, 描述【{step.Description}】");

            var resourceKey = !string.IsNullOrWhiteSpace(task.ResourceId)
                ? task.ResourceId
                : (!string.IsNullOrWhiteSpace(step.Target) ? step.Target : step.Target);
    ...
    /// </summary>
    public class SubFlowLiteExecutor
    {
        /// <summary>
        ...
        /// 默认构造：使用顺序编排器实现。
        /// </summary>
        public SubFlowLiteExecutor() : this(null)
        {
        }
        ...
        /// </summary>
        internal SubFlowLiteExecutor(IOrchestrator orchestrator)
        {
        ...
        private static string ResolveResourceId(StepConfig step, string contextName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(step.Target))
            {
                ...
                PublishPlanLog(
                    $"[BuildPlan] 步骤 {contextName} 未显式配置 ResourceId，默认使用设备名 {step.Target}",
                    warn: false);
                return step.Target;
            }
