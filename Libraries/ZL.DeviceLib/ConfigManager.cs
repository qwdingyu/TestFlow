...
namespace ZL.WorkflowLib
{
    public class ConfigManager
    {
        ...
        private Dictionary<string, DeviceConfig> _allResource = new Dictionary<string, DeviceConfig>();
        ...
        public DeviceConfig? GetResourceByKey(string key)
        {
            if (string.IsNullOrWhiteSpace(key))
                return null;

            if (_allResource?.TryGetValue(key, out var config) == true)
            {
                return config; // 可能返回 null，但调用方需要处理
            }
            return null;
        }

        public bool TryGetResourceByKey(string key, out DeviceConfig? config)
        {
            config = null;
            if (string.IsNullOrWhiteSpace(key) || _allResource == null)
                return false;

            return _allResource.TryGetValue(key, out config) && config != null;
        }
